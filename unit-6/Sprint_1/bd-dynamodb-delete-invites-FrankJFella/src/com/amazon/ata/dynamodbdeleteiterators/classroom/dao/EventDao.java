...
public class EventDao {
    ...
    public Event cancelEvent(String eventId) {
        ...
        // Event canceledEvent = new Event();   // New object to interact with DynamoDB
        Event canceledEvent = this.getEvent(eventId); // get the existing event form the database for update
                                                      //    using this Dao's getEvent() method
                                                      //    this. is optional
        canceledEvent.setId(eventId);           // Set the eventId to the event we want DynamoDB to process

// Actual delete replaced by soft delete in BD5
        //mapper.delete(canceledEvent);        // Delete the event

        // set the isCancelled to true to perform soft delete
        canceledEvent.setCanceled(true);

        // Save the updated event in DynamoDB
